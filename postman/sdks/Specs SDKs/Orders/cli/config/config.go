...
// These keys map to environment variables with the prefix ORDERS_
// (e.g. base_url → ORDERS_BASE_URL).
//
...
var Cmd = &cobra.Command{
	Use:   "config",
	Short: "Manage orders CLI configuration",
	Long: `Manage CLI configuration settings.

Configuration is resolved in the following order (highest priority first):
  1. Environment variables (ORDERS_<KEY>, e.g. ORDERS_BASE_URL)
  2. Config file (./config/config.yaml, relative to where the CLI is invoked)
  3. Saved credentials (~/.orders/credentials, managed by setup-auth)
  4. Built-in defaults
...
var setCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if !isValidKey(key) {
			return fmt.Errorf("unknown config key %q\n\nRun 'orders config --help' to see available keys", key)
		}
