...
**CI:** on PRs a `docs-check` job runs `fern check` (validation only, no publish, no token). The
publishing `docs` job runs **only on push to `main`** — `fern check` then `fern generate --docs`, but
